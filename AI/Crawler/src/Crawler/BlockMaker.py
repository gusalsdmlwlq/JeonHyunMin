...
class BlockMaker:
    ...
    def bindnodes(self, nodelist):
        if type(nodelist) == Node.Node:  # 노드가 하나
            node = nodelist
            block = Block.Block(node.type, node.content, node.x / self.parser.getwidth(), node.y / self.parser.getheight(), node.w, node.h)
        elif len(nodelist) == 1:
            node = nodelist[0]
            block = Block.Block(node.type, node.content, node.x / self.parser.getwidth(), node.y / self.parser.getheight(), node.w, node.h)
        else:   # 노드가 여러개
            ...
            x = x_mean / self.parser.getwidth()
            y_mean = y_sum / len(nodelist)
            y = y_mean / self.parser.getheight()
            first = nodelist[0]
